...
async function invite(actor: AuthUser, listingId: string, input: CoHostInvitationInput) {
  ...
  if (listing.host.email?.toLowerCase() === email || actor.email?.toLowerCase() === email) {
    throw AppError.badRequest("You cannot invite yourself as a co-host");
  }

  await expirePendingForListing(listingId);
  const duplicate = await prisma.listingCoHost.findFirst({
    where: { listingId, email, status: { in: [ListingCoHostStatus.PENDING, ListingCoHostStatus.ACCEPTED] } },
  });
  if (duplicate) throw AppError.conflict("This person already has a pending or accepted co-host invitation");

  const rawToken = randomBytes(32).toString("hex");
  const invitation = await prisma.listingCoHost.create({
    data: {
      listingId,
  ...
  await sendListingCoHostInvitationEmail({
    ...
    invitationUrl: `${appUrl()}/co-host-invitations/accept?token=${encodeURIComponent(rawToken)}`,
  });
...
async function accept(actor: AuthUser, rawToken: string) {
  ...
  if (!actor.email || invitation.email?.toLowerCase() !== actor.email.toLowerCase()) {
    throw AppError.forbidden("Sign in with the email address that received this invitation");
  }
