...
export async function updateHostPermissionOverrideAction(
  ...
  try {
    ...
    const { host, previousEffect } = await hostPermissionService.updateOverride({
      hostId,
...
export async function bulkUpdateHostPermissionsAction(
  ...
  try {
    ...
    const { host } = await hostPermissionService.bulkUpdateOverrides({
      hostId,
...
export async function resetHostPermissionsAction(
  ...
  try {
    ...
    const { host, removedCount } = await hostPermissionService.resetOverrides(hostId);
    ...
    await auditService.record({
      ...
      resourceType: "HOST",
      resourceId: hostId,
      description: `Reset all custom permission overrides (${removedCount} removed) for Host #${hostId} (${host.name || host.email}) to default Host permissions`,
      status: "SUCCESS",
      metadata: {
        hostId,
        hostEmail: host.email,
        removedOverridesCount: removedCount,
        reason: reason?.trim() || null,
      },
    });
