    ...
    >
      ...
      {isTurnOffInstantBookModalOpen && (
        <ModalOverlay className="fixed inset-0 z-50 bg-black/35 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-8 max-w-md w-full space-y-6 shadow-2xl animate-in zoom-in-95 relative">
            {/* Top Close Button */}
            <button
              type="button"
              disabled={isSaving}
              onClick={() => setIsTurnOffInstantBookModalOpen(false)}
              className="absolute top-6 right-6 cursor-pointer p-1 text-xl font-semibold text-[#1f1f1f] hover:text-[#727272] disabled:cursor-wait disabled:opacity-50 transition duration-300"
            >
              ✕
            </button>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-start gap-3.5">
                <div className="w-9 h-9 rounded-full border border-[#727272] bg-white flex items-center justify-center text-zinc-700 shrink-0 shadow-2xs mt-0.5">
      ...
      {isCustomMessageModalOpen && (
        <ModalOverlay className="fixed inset-0 z-50 bg-black/35 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full space-y-5 shadow-2xl animate-in zoom-in-95 relative">
            <button
              type="button"
              disabled={isSaving}
              onClick={() => setIsCustomMessageModalOpen(false)}
              className="absolute top-6 right-6 text-[#1f1f1f] hover:text-[#727272] font-semibold text-lg cursor-pointer p-1 disabled:cursor-not-allowed disabled:opacity-50 transition duration-300"
            >
              ✕
            </button>
