...
export function PhotoTourManager({ photos, onChange, onSave, isSaving, isLoading }: PhotoTourManagerProps) {
  ...
  return <section className="max-w-3xl space-y-5 pb-10">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1>Photo tour</h1><p className="mt-1 text-sm font-normal text-[#727272]">Upload several photos, drag to reorder, and keep your cover photo first.</p></div><button type="button" disabled={isSaving || uploading} onClick={onSave} className="rounded-full bg-[#FCDF9C] hover:bg-[#1F1F1F] hover:text-white text-[#1F1F1F] px-5 py-2 text-sm font-medium disabled:opacity-60 duration-300">{isSaving ? "Saving…" : "Save photo tour"}</button></div>
    ...
    {isLoading ? (
      ...
      <div onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const files = Array.from(event.dataTransfer.files); if (files.length) void uploadFiles(files); }} className="grid grid-cols-2 gap-3 rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-3 sm:grid-cols-3">
        {photos.map((photo, index) => <div key={`${photo}-${index}`} draggable onDragStart={() => setDragIndex(index)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (dragIndex !== null) reorder(dragIndex, index); setDragIndex(null); }} className="group relative aspect-[4/3] overflow-hidden rounded-xl bg-zinc-200"><img src={photo} alt={index === 0 ? "Cover photo" : `Listing photo ${index + 1}`} className="h-full w-full object-cover" /><div className="pointer-events-none absolute inset-0 z-10 flex flex-wrap content-center justify-center gap-2 bg-black/50 p-3 opacity-0 backdrop-blur-[1px] transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100 focus-within:pointer-events-auto focus-within:opacity-100"><button type="button" onClick={() => { const next = [...photos]; const [cover] = next.splice(index, 1); next.unshift(cover); onChange(next); }} className="rounded bg-white px-2 py-1 text-xs font-medium">{index === 0 ? "Cover" : "Make cover"}</button><button type="button" onClick={() => { setReplaceIndex(index); replaceInput.current?.click(); }} className="rounded bg-white px-2 py-1 text-xs font-medium">Replace</button><button type="button" onClick={() => onChange(photos.filter((_, itemIndex) => itemIndex !== index))} className="rounded bg-white px-2 py-1 text-xs font-medium text-rose-700">Delete</button></div></div>)}
          <button type="button" onClick={() => addInput.current?.click()} disabled={uploading} className="flex aspect-[4/3] flex-col items-center justify-center rounded-xl border border-dashed border-zinc-400 bg-white hover:bg-[#E9EBFF] text-xs font-semibold text-zinc-600 disabled:opacity-60 transition-all duration-300"><span className="text-xl">＋</span>Add photos</button>
      </div>
    )}
  </section>;
}
