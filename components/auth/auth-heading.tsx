...
export function AuthHeading({ onBack, title = "Log in or sign up" }: AuthHeadingProps) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-6 mb-2">
      <button type="button" onClick={onBack} className="brush-back-border w-8 h-8 rounded-full bg-[#F3F4F5] flex items-center justify-center text-[#1F1F1F] hover:bg-zinc-200 transition-colors cursor-pointer shrink-0 self-start sm:self-auto" aria-label="Go back">
