              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl border text-sm font-semibold transition-all cursor-pointer select-none ${isSelected
                        ? "border-indigo-400 bg-[#EEF2FF] text-[#1F1F1F] ring-1 ring-indigo-400 shadow-xs"
                        : "border-zinc-200 bg-white text-zinc-800 hover:border-zinc-300 hover:bg-zinc-50"
                      }`}
                  >
                    <div
                      className={`w-7 h-7 rounded-full border flex items-center justify-center shrink-0 transition-colors ${isSelected
