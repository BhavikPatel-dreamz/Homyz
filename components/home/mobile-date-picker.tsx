...
export function MobileDatePicker({
  ...
  const locale = language === "ar" ? "ar-EG" : language === "hi" ? "hi-IN" : language;
  const monthLabel = (date: Date) => date.toLocaleDateString(locale, { month: "long", year: "numeric" });
  ...
  const toggleMonth = (key: string) =>
    update({
      months: preferences.months.includes(key)
        ? preferences.months.filter((month) => month !== key)
        : [...preferences.months, key].sort(),
    });

  const tabKeys = {
    dates: "home_when_tab_dates",
    months: "home_when_tab_months",
    flexible: "home_when_tab_flexible",
  } as const;

  const stayOptions = [
    { key: "Weekend", labelKey: "home_when_stay_weekend" },
    { key: "Week", labelKey: "home_when_stay_week" },
    { key: "Month", labelKey: "home_when_stay_month" },
  ] as const;
        ...
        {preferences.mode === "dates" ? (
                ...
                return (
                  <div key={dateKey(month)} className={offset && !desktop ? "mt-6" : ""}>
                    ...
                    <div className="grid grid-cols-7 text-center text-[12px]">
                      {weekdays.map((day, i) => (
                        <span key={i} className="pb-2 text-[#a09d96]">
                          {day}
                        </span>
                      ))}
            ...
            <div className="no-scrollbar mt-6 flex gap-2 overflow-x-auto">
              {[0, 1, 2, 3, 7].map((days) => (
                <button
                  type="button"
                  key={days}
                  aria-pressed={preferences.tolerance === days}
                  onClick={() => update({ tolerance: days })}
                  className={`${pill} ${
                    preferences.tolerance === days ? "border-[#777] bg-[#f3f4f5]" : "border-[#aaa]"
                  }`}
                >
                  {days === 0
                    ? t("home_when_exact_dates")
                    : t(days === 1 ? "home_when_plus_minus_day" : "home_when_plus_minus_days", { count: days })}
                </button>
              ))}
            </div>
          </>
          ...
          <>
            {preferences.mode === "flexible" && (
              <>
                <h4 className="mb-4 mt-1 text-center text-[16px] font-medium">{t("home_when_how_stay")}</h4>
                <div className="flex justify-center gap-2">
                  {stayOptions.map(({ key, labelKey }) => (
                    <button
                      key={key}
                      type="button"
                      aria-pressed={preferences.stay === key}
                      onClick={() => update({ stay: key })}
                      className={`${pill} ${
                        preferences.stay === key ? "border-[#777] bg-[#f3f4f5]" : "border-[#aaa]"
                      }`}
                    >
                      {t(labelKey)}
                    </button>
            ...
            <h4 className="mb-4 mt-7 text-center text-[16px] font-medium">
              {preferences.mode === "flexible" ? t("home_when_go_anytime") : t("home_when_which_months")}
            </h4>
            <div className="flex items-center gap-2">
              {desktop && (
                <button
                  ...
                  aria-controls={`${pickerId}-months`}
                  disabled={sliderEdges.start}
                  onClick={() => slideMonths(-1)}
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#aaa] hover:bg-[#f3f4f5] disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
                >
                ...
                {Array.from({ length: 12 }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      aria-pressed={preferences.months.includes(key)}
                      onClick={() => toggleMonth(key)}
                      className={`flex min-w-0 shrink-0 snap-start flex-col items-center rounded-[22px] border px-0.5 py-4 cursor-pointer ${
