    ...
    <div className="flex flex-col items-center justify-center rounded-3xl border border-[var(--error)]/30 bg-[var(--error)]/5 p-10 text-center my-6">
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-xs font-semibold text-muted-foreground hover:bg-[var(--muted)] transition-colors shadow-2xs"
        >
          Try Again
        </button>
      )}
